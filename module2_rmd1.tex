...
Here is a link to \href{www.google.com}{GOOGLE}

Here is a word in \textbf{bold} and another word in \textbf{bold}.

Here in another word in \emph{italics} and another word in
\emph{italics}.

Use backticks to get non-proportional font in light grey as
\texttt{rmarkdown}.
...
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{head}\NormalTok{(cars),}
             \DataTypeTok{caption =} \StringTok{'Table 1. Top ten rows of the cars dataset'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable}[]{@{}rr@{}}
\caption{Table 1. Top ten rows of the cars dataset}\tabularnewline
\toprule
...
1\tabularnewline
\#\# Insert an Equatio & n & & & & & & & & & &\tabularnewline
\bottomrule
\end{longtable}
...
{[}sunstar\_annimated\_gig{]}

\begin{figure}
\centering
\includegraphics{sunstar.mp4}
\caption{sunstar\_mp4}
\end{figure}
